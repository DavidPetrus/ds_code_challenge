...
flags.DEFINE_integer('num_workers',128,'')
...
def main(argv):
    ...
    for epoch in range(FLAGS.num_epochs):
        for item in trainloader:
            ...
            if train_iter % 100 == 1:
                print(f"Epoch: {epoch}, Train Iter: {train_iter:.2f}, Loss: {loss:.2f}, Det Pools: {det_pools:.2f}, Miss Pools: {miss_pools:.2f}, False Pos: {false_pools:.2f}")
        ...
        for item in valloader:
            with torch.no_grad():
                ...
                pool_infers = torch.where(result > FLAGS.pool_thresh, 1., 0.)
                pool_infers = pool_infers.max(dim=1) # bs
                ...
                if val_iter % 20 == 1:
                    print(f"Val Iter: {val_iter:.2f}, Loss: {loss:.2f}, Det Pools: {det_pools:.2f}, Miss Pools: {miss_pools:.2f}, False Pos: {false_pools:.2f}")
